...
flowchart TD
    ...
    J[" id: 10
        ...
        attributes:
        service.name: notification-api
        qualitytrace.span.type: messaging
    "]
    K[" id: 10
...
```css
span[qualitytrace.span.type="http" service.name="cart-api"]
```
...
```css
span[qualitytrace.span.type="http"]:first
```
...
```css
span[qualitytrace.span.type="http"]:last
```
...
```css
span[qualitytrace.span.type="http"]:nth_child(3)
```
...
```css
span[service.name="auth-api" qualitytrace.span.type="http"]
```
...
```css
span[service.name="cart-api", name="purchase products"] span[service.name="auth-api" qualitytrace.span.type="http"]
```
