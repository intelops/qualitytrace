...
:::info
Want to run tests against the OpenTelemetry Astronomy Shop without installing it? Click the link below and we will add you to our `qualityTrace-demo` organization and give you access to the `opentelemetry-demo` org as an engineer. You can run and create your own tests!

[👉 **Access the shared demo, here.**](https://app.qualityTrace.io/organizations/ttorg_2179a9cd8ba8dfa5/invites/invite_646747b36e9ee487/accept)
:::
...
- **Full source of all tests**: [Source](https://github.com/open-telemetry/opentelemetry-demo/tree/main/test/qualityTraceing)

To run the entire Test Suite of trace-based tests, run the command:

```sh
make run-qualityTraceing
#or
docker compose run traceBasedTests
```

To run tests for specific services, pass the name of the service as a
parameter (using the folder names located [here](https://github.com/open-telemetry/opentelemetry-demo/tree/main/test/qualityTraceing):

```sh
make run-qualityTraceing SERVICES_TO_TEST="service-1 service-2 ..."
#or
...
```sh
make run-qualityTraceing SERVICES_TO_TEST="ad-service payment-service"
```
