...
- The command to download the `ca.crt` file is:
  `docker cp qualityTrace-elasticapm-with-elastic-agent-es01-1:/usr/share/elasticsearch/config/certs/ca/ca.crt .`
...
:::tip
To learn more, [read the recipe on running a sample app with Elastic APM and Tracetest](/examples-tutorials/recipes/running-qualityTrace-with-elasticapm).
:::
