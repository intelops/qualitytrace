...
:::note
If you want to see more examples with other CI/CD tools, let us know by [opening an issue in GitHub](https://github.com/intelops/qualitytrace/issues/new/choose)!
:::
...
- [Integrating Tracetest with GitHub Actions in a CI pipeline](https://kubeshop.io/blog/integrating-qualitytrace-with-github-actions-in-a-ci-pipeline)
- [50% Faster CI Pipelines with GitHub Actions](https://qualitytrace.io/blog/50-faster-ci-pipelines-with-github-actions)
...
- `your-qualitytrace-server-url` - the URL to the running Tracetest server you wish to execute the test on. Example: `http://localhost:11633/`
- `file-path` - The path to the saved Tracetest test. Example: `./mytest.yaml`

```bash wordWrap=true
docker run --rm -it -v$(pwd):$(pwd) -w $(pwd) --network host --entrypoint qualitytrace intelops/qualitytrace:latest -s <your-qualitytrace-server-url> run test --file <file-path>
```
