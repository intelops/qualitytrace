...
You can find the latest version [here](https://github.com/intelops/qualityTrace/releases/latest).
...
# update and install
sudo apt-get update
sudo apt-get install qualityTrace
```
...
# install
sudo yum install qualityTrace --refresh
```
...
Download one of the files from the latest tag, extract to your machine, and then [add the qualityTrace binary to your PATH variable](https://stackoverflow.com/a/41895179).
...
```bash
curl -L https://raw.githubusercontent.com/intelops/qualityTrace/main/install-cli.sh | bash -s -- [version]
```

If you would want version `v0.13.0` you would run this command:

```bash
curl -L https://raw.githubusercontent.com/intelops/qualityTrace/main/install-cli.sh | bash -s -- v0.12.1
```
