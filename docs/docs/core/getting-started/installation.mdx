...
- Using the [Docker Compose Quick Start with the Pokeshop Sample App](https://github.com/intelops/qualitytrace/tree/main/examples/quick-start-pokeshop).
...
Choosing any option, this installer will create a `qualitytrace` directory in the current directory and add a `docker-compose.yaml` file to it.
...
**Start Docker Compose from the directory where you ran `qualitytrace server install`.**

<CodeBlock
  language="bash"
  title="Terminal"
>
{`docker compose -f qualitytrace/docker-compose.yaml up -d`}
</CodeBlock>
...
Choosing any option, this installer will create a `qualitytrace` namespace in the Kubernetes context you choose and create deployments for Tracetest Core and its dependencies.

- If you choose the first option, the `qualitytrace` namespace will only contain Tracetest Core and its dependencies.
...
kubectl --kubeconfig <path-to-your-home>/.kube/config --context <your-cluster-context> --namespace qualitytrace port-forward svc/qualitytrace 11633
...
The Quick Start with the Pokeshop Sample App is located [here](https://github.com/intelops/qualitytrace/tree/main/examples/quick-start-pokeshop).

**The [`docker-compose.yaml`](https://github.com/intelops/qualitytrace/blob/main/examples/quick-start-pokeshop/docker-compose.yml) contains Tracetest Core, the OpenTelemetry Collector, and a sample app called [Pokeshop](/live-examples/pokeshop/overview). This allows you to create sample tests right away!**
