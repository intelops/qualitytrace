...
description: Tracetest Core requires a minimal configuration to be launched. Connect to a PostgreSQL database which is installed as part of the "qualitytrace server install" command.
keywords:
  - qualitytrace
  - trace-based testing
  - observability
...
For Docker-based installs, the server configuration file is placed in the `./qualitytrace/qualitytrace.yaml` file by default when you run the `qualitytrace server install` command and select the `Using Docker Compose` option. The configuration file is mounted to `/app/config.yaml` within the Tracetest Docker container. When Tracetest is run with a `docker compose -f qualitytrace/docker-compose.yaml  up -d` command, the server will use the contents of this file to connect to the Postgres database. All other configuration data is stored in the Postgres instance.

This is an example of a `qualitytrace.yaml` file:
