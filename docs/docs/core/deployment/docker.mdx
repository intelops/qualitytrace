...
- `qualitytrace/` directory (configurable) with a `docker-compose.yaml` and other config files.
...
In the folder where you ran `qualitytrace server install`, run this command to start the Tracetest cluster:
