...
You access the Tracetest Web UI on [`app.qualitytrace.io`](https://app.qualitytrace.io). Here's what will greet you after a fresh install.
...
```bash title=Terminal
git clone --depth 1 https://github.com/intelops/qualitytrace.git
cd qualitytrace/examples/qualitytrace-agent/pokeshop/
docker-compose up
```
...
```bash title="Terminal"
qualitytrace run test -f pokemon-import.yaml --required-gates test-specs --output pretty
```
...
```text title="Output"
✔ Pokemon Import (https://app.qualitytrace.io/organizations/<org_id>/environments/<env_id>/test/T7-delWIR/run/7/test) - trace id: <trace_id>
