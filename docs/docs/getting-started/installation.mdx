...
This page showcases getting started with [Tracetest](https://app.qualitytrace.io/) by using the Tracetest CLI and Tracetest Agent.
...
[Sign in to Tracetest](https://app.qualitytrace.io/), and get your [Tracetest Agent API Key](/configuration/agent).
...
<CodeBlock
  ...
  title="Terminal"
>
{`qualitytrace start --api-key <your_api_key>`}
</CodeBlock>
...
1. Run Tracetest Agent locally with the `qualitytrace start` command.
...
<CodeBlock
    ...
    title="Terminal"
>
{`curl https://raw.githubusercontent.com/intelops/qualitytrace/main/k8s/agent/deploy-agent.sh | bash -s -- default <your_api_key>`}
</CodeBlock>
